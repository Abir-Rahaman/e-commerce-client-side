...
const UserRow = ({user,index,refetch,setDeleteAdmin}) => {
    ...
    return (
        ...
        <td>{email}</td>
        <td>
         { role? <span className="font-bold text-green-400 ml-4">Admin</span> :   <button onClick={makeAdmin} class="btn btn-xs text-white bg-red-500 border-none">Make Admin</button> }
        </td>
        <td>
